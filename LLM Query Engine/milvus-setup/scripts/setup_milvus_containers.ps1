...
$milvusSetupDir = Split-Path -Parent $scriptDir

# Change to milvus-setup directory
Set-Location $milvusSetupDir
Write-Host "Running from directory: $milvusSetupDir" -ForegroundColor Cyan
...
if (-not (Test-Path "docker-compose.yml")) {
    Write-Host "docker-compose.yml not found in milvus-setup directory." -ForegroundColor Red
    Write-Host "Please ensure this script is in the milvus-setup/scripts directory." -ForegroundColor Yellow
    exit 1
}
